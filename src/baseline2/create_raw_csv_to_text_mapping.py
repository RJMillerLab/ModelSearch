...
def to_parquet(df, output_path, **kwargs):
    """Save DataFrame to Parquet (inlined from utils for portability)."""
    df.to_parquet(output_path, index=False, **kwargs)
    return output_path


def _html_path_to_relative(html_path):
    """Convert absolute html_path (e.g. /Users/.../ModelTables/data/arxiv_fulltext_html_251117/xxx.html) to relative so parquet is portable."""
