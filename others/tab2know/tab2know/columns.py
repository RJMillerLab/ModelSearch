...
class SupervisedPipeline:
        ...
        for meta, rows in zip(metadatas, tables):
            ...
            for ci, col in enumerate(cols):
                ...
                if unlabeled or y:
                    ...
                    # add features
                    # Optional: drop caption features without changing raw data
                    # Enable by setting environment variable TAB2KNOW_NO_CAPTION=1
                    no_caption = bool(os.environ.get('TAB2KNOW_NO_CAPTION'))
                    caption_text = '' if no_caption else (meta.get('dc:title', '') or '')
                    
                    x = {
                        '@id':
                        meta.get('@id') + f'#col={ci+1}',
                        'tableCaption':
                        process_text(caption_text),
                        'headerText':
    ...
    def transform(self, meta, rows, **kwargs):
        records = list(SupervisedPipeline.make_records([meta], [rows], unlabeled=True))
        if not records:
            # Skip if no records generated (empty table or no columns)
            return meta
        X, y = zip(*records)
        X = pd.DataFrame.from_records(X).set_index('@id')
        X = X.replace([pd.np.inf, -pd.np.inf, pd.np.nan], 0)
        # Ensure compatibility with pipelines trained on a combined 'text' feature
        if 'text' not in X.columns:
            for col in ['tableCaption', 'headerText', 'otherHeaderText', 'columnText']:
                if col not in X.columns:
                    X[col] = ''
            X['text'] = (
                X['tableCaption'].astype(str) + ' ' +
                X['headerText'].astype(str) + ' ' +
                X['otherHeaderText'].astype(str) + ' ' +
                X['columnText'].astype(str)
            )
